...
function(target_enable_linux_standalone_package target)
  if(TARGET ${target}_Standalone)
    ...
    if(ICON_BIG_FILE_STANDALONE)
      target_add_copy_command(${CJAP_PACKAGE_PROJECT_NAME}_Package "${ICON_BIG_FILE_STANDALONE}" "${CJAP_PACKAGE_INSTALL_DIR}")
      get_filename_component(ICON_BIG_FILE_STANDALONE_NAME "${ICON_BIG_FILE_STANDALONE}" NAME)
      file(APPEND "${CJAP_PACKAGE_INSTALL_DIR}/install.sh" "cp -f $ThisPath/'${ICON_BIG_FILE_STANDALONE_NAME}' '/opt/${PLUGIN_NAME}'\n")
    elseif(ICON_SMALL_FILE_STANDALONE)
      target_add_copy_command(${CJAP_PACKAGE_PROJECT_NAME}_Package "${ICON_SMALL_FILE_STANDALONE}" "${CJAP_PACKAGE_INSTALL_DIR}")
      get_filename_component(ICON_SMALL_FILE_STANDALONE_NAME "${ICON_SMALL_FILE_STANDALONE}" NAME)
      file(APPEND "${CJAP_PACKAGE_INSTALL_DIR}/install.sh" "cp -f $ThisPath/'${ICON_SMALL_FILE_STANDALONE_NAME}' '/opt/${PLUGIN_NAME}'\n")
    endif()
...
function(target_enable_linux_cjap_package target)
  if(CJAP_PACKAGE_ENABLED AND UNIX AND NOT APPLE)
    ...
    if(PACKAGE_EXTRA_ARGS)
      file(APPEND "${CJAP_PACKAGE_INSTALL_DIR}/install.sh" "${PACKAGE_EXTRA_ARGS}")
    endif()
  endif()
...
function(linux_cjap_package_add_file file destination)
  if(CJAP_PACKAGE_ENABLED AND UNIX AND NOT APPLE)
    ...
    file(APPEND ${CJAP_PACKAGE_INSTALL_DIR}/install.sh "mkdir -p ${destination}\n")
