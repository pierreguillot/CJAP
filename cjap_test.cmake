...
set(CJAP_TEST_ENVIRONMENT "" CACHE STRING "The environment variables to set for the tests")
...
if(APPLE AND "${CMAKE_OSX_ARCHITECTURES}" STREQUAL "x86_64" AND NOT "${CMAKE_HOST_SYSTEM_PROCESSOR}" STREQUAL "x86_64")
  set(CJAP_TEST_APPLE_CROSSCOMPILING ON)
elseif(APPLE AND "${CMAKE_OSX_ARCHITECTURES}" STREQUAL "arm64" AND NOT "${CMAKE_HOST_SYSTEM_PROCESSOR}" STREQUAL "arm64")
  set(CJAP_TEST_APPLE_CROSSCOMPILING ON)
else()
  set(CJAP_TEST_APPLE_CROSSCOMPILING OFF)
endif()
...
elseif(CJAP_TEST_APPLE_CROSSCOMPILING)
  # AUVAL validator is not supported when cross-compiling
  set(CJAP_TEST_AUVAL_ENABLED OFF)
elseif(WIN32)
  # AAX and AUVAL validators are not supported on Windows (but AAX should be)
  set(CJAP_TEST_AAXVALIDATOR_ENABLED OFF)
  set(CJAP_TEST_AUVAL_ENABLED OFF)
endif()
...
function(cjap_test_check_or_download_vst3_sdk)
  if(EXISTS ${CJAP_TEST_VST3_SDK_DIR})
    ...
    execute_process(COMMAND git clone --quiet -c advice.detachedHead=false --depth=1 --shallow-submodules --recursive --single-branch --branch v3.7.12_build_20 https://github.com/steinbergmedia/vst3sdk.git "${CJAP_TEST_VST3_SDK_DIR}" RESULT_VARIABLE DL_RESULT OUTPUT_QUIET)
    if(DL_RESULT)
...
if(CJAP_TEST_PLUGINVAL_ENABLED)
  ...
  if(NOT CJAP_TEST_PLUGINVAL_EXE)
    if(APPLE)
      file(DOWNLOAD "https://github.com/Tracktion/pluginval/releases/download/v1.0.3/pluginval_macOS.zip" "${CJAP_TEST_BINARY_DIR}/pluginval.zip" STATUS DL_RESULT)
    elseif(UNIX)
      file(DOWNLOAD "https://github.com/Tracktion/pluginval/releases/download/v1.0.3/pluginval_Linux.zip" "${CJAP_TEST_BINARY_DIR}/pluginval.zip" STATUS DL_RESULT)
    elseif(WIN32)
      file(DOWNLOAD "https://github.com/Tracktion/pluginval/releases/download/v1.0.3/pluginval_Windows.zip" "${CJAP_TEST_BINARY_DIR}/pluginval.zip" STATUS DL_RESULT)
    endif()
    list(GET DL_RESULT 0 DL_RESULT_CODE)
    if(${DL_RESULT_CODE})
      list(GET DL_RESULT 1 DL_RESULT_ERROR)
      message(FATAL_ERROR "The pluginval binaries could not be retrieved: ${DL_RESULT_ERROR}")
    endif()
...
if(CJAP_TEST_ARATESTHOST_ENABLED)
  ...
  if(NOT APPLE OR CMAKE_GENERATOR STREQUAL "Xcode")
    set(CJAP_CAN_GEN_ARATESTHOST_EXE ON)
  else()
    set(CJAP_CAN_GEN_ARATESTHOST_EXE OFF)
  endif()
  if(NOT CJAP_TEST_ARATESTHOST_EXE AND CJAP_CAN_GEN_ARATESTHOST_EXE)
    cjap_test_check_or_download_vst3_sdk()
    if(APPLE)
      cjap_test_check_or_download_au_sdk()
      execute_process(COMMAND ${CMAKE_COMMAND} . -B "${CJAP_TEST_BINARY_DIR}/AraTestHost" -G ${CMAKE_GENERATOR} -DCMAKE_OSX_ARCHITECTURES=${CMAKE_OSX_ARCHITECTURES} -DCMAKE_XCODE_ATTRIBUTE_CODE_SIGNING_ALLOWED=NO -DARA_VST3_SDK_DIR=${CJAP_TEST_VST3_SDK_DIR} -DARA_AUDIO_UNIT_SDK_DIR=${CJAP_TEST_AUDIOUNIT_SDK_DIR} -DARA_SETUP_DEBUGGING=OFF -DSMTG_ENABLE_VSTGUI_SUPPORT:BOOL=OFF -DSMTG_ENABLE_VST3_HOSTING_EXAMPLES:BOOL=OFF -DSMTG_ENABLE_VST3_PLUGIN_EXAMPLES:BOOL=OFF -Wno-deprecated WORKING_DIRECTORY ${CJAP_TEST_ARA_SDK_DIR}/ARA_Examples RESULT_VARIABLE DL_RESULT OUTPUT_QUIET)
    elseif(UNIX)
      execute_process(COMMAND ${CMAKE_COMMAND} . -B "${CJAP_TEST_BINARY_DIR}/AraTestHost" -G ${CMAKE_GENERATOR} -DCMAKE_BUILD_TYPE=Release -DARA_VST3_SDK_DIR=${CJAP_TEST_VST3_SDK_DIR} -DARA_SETUP_DEBUGGING=OFF -DSMTG_ENABLE_VSTGUI_SUPPORT:BOOL=OFF -DSMTG_ENABLE_VST3_HOSTING_EXAMPLES:BOOL=OFF -DSMTG_ENABLE_VST3_PLUGIN_EXAMPLES:BOOL=OFF -Wno-deprecated WORKING_DIRECTORY "${CJAP_TEST_ARA_SDK_DIR}/ARA_Examples" RESULT_VARIABLE DL_RESULT OUTPUT_QUIET)
    elseif(WIN32)
...
if(CJAP_TEST_VST3VALIDATOR_ENABLED)
  ...
  if(NOT CJAP_TEST_VST3VALIDATOR_EXE)
    if(UNIX)
      execute_process(COMMAND ${CMAKE_COMMAND} -B "${CJAP_TEST_BINARY_DIR}/Vst3Validator" -G ${CMAKE_GENERATOR} -DCMAKE_OSX_ARCHITECTURES=${CMAKE_OSX_ARCHITECTURES} -DSMTG_ENABLE_VSTGUI_SUPPORT=OFF -DSMTG_ENABLE_VST3_HOSTING_EXAMPLES=OFF -DSMTG_ENABLE_VST3_PLUGIN_EXAMPLES=OFF -Wno-deprecated WORKING_DIRECTORY ${CJAP_TEST_VST3_SDK_DIR} OUTPUT_QUIET)
    elseif(WIN32)
      execute_process(COMMAND ${CMAKE_COMMAND} -B "${CJAP_TEST_BINARY_DIR}/Vst3Validator" -G ${CMAKE_GENERATOR} -A ${CMAKE_GENERATOR_PLATFORM} -DSMTG_ENABLE_VSTGUI_SUPPORT=OFF -DSMTG_ENABLE_VST3_HOSTING_EXAMPLES=OFF -DSMTG_ENABLE_VST3_PLUGIN_EXAMPLES=OFF -Wno-deprecated WORKING_DIRECTORY ${CJAP_TEST_VST3_SDK_DIR} OUTPUT_QUIET)
    endif()
...
if(CJAP_TEST_AAXVALIDATOR_ENABLED)
  ...
  if(NOT CJAP_TEST_AAX_DTT_EXE)
    ...
    if(APPLE AND "${CMAKE_OSX_ARCHITECTURES}" MATCHES "arm64")
      get_filename_component(CJAP_TEST_AAXVALIDATOR_PNAME ${CJAP_TEST_AAXVALIDATOR_URL_APPLE_arm64} NAME)
      file(DOWNLOAD ${CJAP_TEST_AAXVALIDATOR_URL_APPLE_arm64} "${CJAP_TEST_AAX_BINARY_DIR}/${CJAP_TEST_AAXVALIDATOR_PNAME}" STATUS DL_RESULT)
    elseif(APPLE AND "${CMAKE_OSX_ARCHITECTURES}" MATCHES "x86_64")
      get_filename_component(CJAP_TEST_AAXVALIDATOR_PNAME ${CJAP_TEST_AAXVALIDATOR_URL_APPLE_x86_64} NAME)
      file(DOWNLOAD ${CJAP_TEST_AAXVALIDATOR_URL_APPLE_x86_64} "${CJAP_TEST_AAX_BINARY_DIR}/${CJAP_TEST_AAXVALIDATOR_PNAME}" STATUS DL_RESULT)
    endif()
    list(GET DL_RESULT 0 DL_RESULT_CODE)
    if(${DL_RESULT_CODE})
      list(GET DL_RESULT 1 DL_RESULT_ERROR)
    ...
    file(ARCHIVE_EXTRACT INPUT "${CJAP_TEST_AAX_BINARY_DIR}/${CJAP_TEST_AAXVALIDATOR_PNAME}" DESTINATION "${CJAP_TEST_AAX_BINARY_DIR}")
  ...
  if(CJAP_TEST_AAX_DTT_EXE)
    ...
    function(add_aax_test test_id)
      file(APPEND "${CJAP_TEST_AAXVALIDATOR_EXE}" "${CJAP_TEST_AAX_DTT_EXE} --script '4' -a 'pi_path='$1'' -a 'result_format=json' -a 'out_path=${CJAP_TEST_BINARY_DIR}/aax_tests' -a 'test_id=${test_id} -v'\n")
...
function(target_enable_cjap_test_vst3 target)
  if(TARGET ${target}_VST3)
    ...
    if(CJAP_TEST_PLUGINVAL_EXE)
      if(CJAP_TEST_APPLE_CROSSCOMPILING)
        add_test(NAME PluginVal_Test_${target}_VST3 COMMAND arch -${CMAKE_OSX_ARCHITECTURES} ${CJAP_TEST_PLUGINVAL_EXE} ${CJAP_TEST_PLUGINVAL_VST3_ARGS} --validate "${ARTEFACT_FILE_VST3}")
      else()
      ...
      set_tests_properties(PluginVal_Test_${target}_VST3 PROPERTIES ENVIRONMENT "${CJAP_TEST_ENVIRONMENT}")
    endif()
    ...
    if(CJAP_TEST_ARATESTHOST_EXE AND ${CJAP_TEST_VST3_IS_PLUGIN_ARA_EFFECT})
      ...
      set_tests_properties(AraTestHost_Test_${target}_VST3 PROPERTIES ENVIRONMENT "${CJAP_TEST_ENVIRONMENT}")
    endif()
  endif()
...
function(target_enable_cjap_test_audiounit target)
  if(CJAP_TEST_AUVAL_EXE AND APPLE AND TARGET ${target}_AU)
    ...
    if(NOT CJAP_TEST_APPLE_CROSSCOMPILING)
      ...
      set_tests_properties(AUVal_Test_${target}_AU PROPERTIES ENVIRONMENT "${CJAP_TEST_ENVIRONMENT}")
      if(CJAP_TEST_PLUGINVAL_EXE)
        ...
        set_tests_properties(PluginVal_Test_${target}_AU PROPERTIES ENVIRONMENT "${CJAP_TEST_ENVIRONMENT}")
      endif()
    endif()
    ...
    if(CJAP_TEST_ARATESTHOST_EXE AND ${CJAP_TEST_AU_IS_PLUGIN_ARA_EFFECT})
      ...
      set_tests_properties(AraTestHost_Test_${target}_AU PROPERTIES ENVIRONMENT "${CJAP_TEST_ENVIRONMENT}")
    endif() 
  endif()
...
function(target_enable_cjap_test_aax target)
  if(TARGET ${target}_AAX AND CJAP_TEST_AAXVALIDATOR_EXE)
    ...
    if(CJAP_TEST_APPLE_CROSSCOMPILING)
      add_test(NAME AAXValidator_Test_${target}_AAX COMMAND arch -${CMAKE_OSX_ARCHITECTURES} ${CJAP_TEST_AAXVALIDATOR_EXE} '${ARTEFACT_FILE_AAX}')
    else()
      add_test(NAME AAXValidator_Test_${target}_AAX COMMAND ${CJAP_TEST_AAXVALIDATOR_EXE} '${ARTEFACT_FILE_AAX}')
    endif()
    set_tests_properties(AAXValidator_Test_${target}_AAX PROPERTIES PASS_REGULAR_EXPRESSION "E_COMPLETED_PASS")
    set_tests_properties(AAXValidator_Test_${target}_AAX PROPERTIES ENVIRONMENT "${CJAP_TEST_ENVIRONMENT}")
  endif()
