...
if(CJAP_PACKAGE_ENABLED AND APPLE)
  ...
  file(APPEND "${CJAP_PACKAGE_PACKAGER_FILE_PATH}" "    sourcedir=\$3\n")
  file(APPEND "${CJAP_PACKAGE_PACKAGER_FILE_PATH}" "    sourcename=\$4\n")
  file(APPEND "${CJAP_PACKAGE_PACKAGER_FILE_PATH}" "    pkgid=\$5\n")
  file(APPEND "${CJAP_PACKAGE_PACKAGER_FILE_PATH}" "    pkgdestination=\$6\n")
  file(APPEND "${CJAP_PACKAGE_PACKAGER_FILE_PATH}" "    extraargs=\$7\n")
  ...
  file(APPEND "${CJAP_PACKAGE_PACKAGER_FILE_PATH}" "    pkgbuild --analyze --root \"$pkgdir\" \"${CJAP_PACKAGE_BUILD_PATH}/$name.plist\"\n")
  ...
  file(APPEND "${CJAP_PACKAGE_PACKAGER_FILE_PATH}" "    pkgbuild $extraargs ${SIGN_CMD} --component-plist \"${CJAP_PACKAGE_BUILD_PATH}/$name.plist\" --root \"$pkgdir\" --identifier \"\$pkgid\" --version \"\$pkgversion\" --install-location \"\$pkgdestination\" \"\$pkgtarget\" || exit 1\n")
  if(CJAP_CODESIGN_ENABLED)
  ...
  add_custom_target(${CJAP_PACKAGE_PROJECT_NAME}_Package ALL ${CJAP_PACKAGE_GENERATOR_FILE_PATH} "$CONFIGURATION" BYPRODUCTS ${CJAP_PACKAGE_INSTALL_DIR}/${CJAP_PACKAGE_PROJECT_NAME}.pkg)
...
function(target_enable_apple_generic_package target tformat format destination)
  if(TARGET ${target}_${tformat})
    ...
    file(APPEND "${CJAP_PACKAGE_PACKAGER_FILE_PATH}" "build_target_format_pkg \"${PACKAGE_NAME}\" \"${format}\" \"${PLUGIN_ARTEFACT_DIR}\" \"${PLUGIN_ARTEFACT_FILE}\" \"${PACKAGE_ID}\" \"${destination}\" \"${PACKAGE_EXTRA_ARGS}\"\n")
...
function(target_enable_apple_all_package target)
  ...
  if(TARGET ${target}_AU)
    target_enable_apple_generic_package(${target} "AU" "AU" "/Library/Audio/Plug-Ins/Components")
  endif()
